...
**Эвокод 0.9.0** — рабочая privacy-first AI-IDE: брендированный VSCodium («Эвокод») с изолированной визуальной средой для операторов (предпросмотр HTML/MD по умолчанию), полностью защищенным локальным Core на `:8083` (DLP, авторизация, лимиты, безопасный импорт навыков), Midnight Fusion интерфейсом, умным роутером бесплатных моделей OpenRouter (`openrouter-auto`) и готовыми дистрибутивами deb + AppImage.

Это **релиз-кандидат (Release Candidate 1)** перед выходом стабильной версии 1.0.0. Система полностью готова к пилотным испытаниям и защищена от утечек.
