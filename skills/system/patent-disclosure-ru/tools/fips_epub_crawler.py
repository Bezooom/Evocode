# -*- coding: utf-8 -*-
"""
ФИПС (Роспатент) — http://fips.ru/ — **поиск публикаций** (форма поиска / #searchStr).

Требуется **Playwright + Chromium**. Если нужен только парсинг в памяти, без сохранения HTML — приоритетно
**`fips_epub_search.py`**; этот файл фокусируется на **записи HTML результатов** и API ``fetch_epub_result_html``.
...
8. Последующий парсинг — **`fips_epub_parse.py`** (внутри ``search_epub_keyword`` вызывается).

-------------------------------------------------------------------------------
二、策略摘要：在解决什么、用了哪些手段
-------------------------------------------------------------------------------
- **为何用 Playwright**：站点依赖 **浏览器内 JavaScript** 渲染与风控后再开放检索框；**纯 HTTP 抓取**往往拿不到含 ``#searchStr`` 的可用首页或拿不到真实结果 DOM。
- **Что значит "обход"**: технический разрыв между headless-автоматизацией и статическим парсингом — через **реальный Chromium + ожидание JS + обычные браузерные отпечатки** (UA, язык, viewport) снижение вероятности неудачи при **сразу отправке формы**; **не** означает обход законодательства или условий сервиса; использование только для легального поиска и помощи в проверке новизны заявки.
...
三、检索关键词建议
-------------------------------------------------------------------------------
- ФИПС-поиск обычно интерпретирует **несколько терминов** как **AND**; **много специфичных терминов** часто даёт **0 результатов**; **рекомендуется** каждый раз использовать **один термин или очень короткую фразу**; для широкого охвата — ``fips_epub_search.py`` (разделение пробелами, множественные поиски и объединение), или ручной перебор.
- Командная строка по умолчанию принимает одну строку (может содержать пробелы); с пробелами — аналогично одной браузерной отправке, семантика **AND**, не эквивалентна раздельным поискам.

-------------------------------------------------------------------------------
环境变量
-------------------------------------------------------------------------------
- **Окружение**: ``EPUB_WAF_MAX_WAIT_SEC`` — макс. время ожидания #searchStr, по умолчанию 180; ``PLAYWRIGHT_HEADED`` — 1 для GUI Chromium; ``EPUB_RESULT_HTML`` — полный путь HTML результатов; не задан — ``tools/_last_result_YYYYMMDDHHmmss.html``
"""
...
from fips_epub_parse import EpubSearchHit, hits_to_jsonable, parse_search_result_html


def _ensure_utf8_stdio() -> None:
    """Для избежания кракозябр JSON на Windows (аналогично fips_epub_search.py)."""
...
def _safe_page_content(page: Page, *, max_attempts: int = 10) -> str:
    ...
    if last_err:
        raise last_err
       raise RuntimeError("_safe_page_content: не вернул контент")
...
def fetch_epub_result_html(
    ...
    *,
    ...
    """
    Только загрузка HTML результатов поиска, не парсинг.
    Парсинг используйте ``fips_epub_parse.parse_search_result_html(html)``.
    """
...
def _dump_home_debug() -> None:
    """Отладка: только загрузка главной страницы и сохранение HTML после WAF."""
...
if __name__ == "__main__":
    ...
    if argv and argv[0] in ("--dump-home", "-d"):
        _dump_home_debug()
        sys.exit(0)
    kw = (argv[0] if argv else "поиск").strip()
    try:
    ...
    print("HTML результатов сохранён:", out_path, file=sys.stderr, flush=True)
    print(
        "EPUB_HITS_JSON:",
