#!/usr/bin/env python3
"""
Последовательно рендерит fenced mermaid-блоки (`` ```mermaid`` ``) в PNG,
и заменяет их на ``![](относительный_путь)`` в выходном Markdown, для встраивания в Word через ``md_to_docx.py``.

Порядок поиска **mmdc** см. в ``_find_mmdc_invocation``:
1. ``tools/node_modules`` (официальный ``npm install @mermaid-js/mermaid-cli``);
2. **mmdc в PATH** (обычно ``npm install -g @mermaid-js/mermaid-cli``);
3. **Node.js + npx** временная установка ``@mermaid-js/mermaid-cli`` (если нет локальной установки).

Блок-схема системы **3.2** и блок-схема процесса **3.4** — fenced mermaid; **не** ASCII-схемы.

**Понижение**: если один блок не рендерится, **не останавливается** —围栏 сохраняется; остальные рендерятся нормально. .md пишется, и **по-прежнему** пытается ``md_to_docx.py`` (в Word блок будет как код).

**Чёткость**: по умолчанию передаётся большой viewport (``-w`` / ``-H``) и ``-s 2`` (пиксельная плотность Puppeteer), PNG в Word при ширине ~5.5 дюймов более резкий. Можно увеличить через ``--mmdc-scale 3`` (файл больше).

Использование:
  python tools/mermaid_render.py -i draft.md -o disclosure.md
  # по умолчанию в том же каталоге — disclosure.docx; при ошибке stderr покажет md_to_docx
